# phedev_menu.py
# Build: pyinstaller --onefile --windowed --add-data "phedev.ico;." phedev_menu.py
...
from PyQt6.QtGui import (
    ...
    QPainterPath, QPolygonF, QImage, QMovie
)
import psutil
import win32gui
import win32con
...
# ===== PHEDEV KEY SERVER (MẠNH HƠN) =====
class PhedevKeyServer:
    BASE_URL = "https://foxone.freedev.app/"  # ← phải có dấu =
    NET_TIMEOUT = 5000
    MAX_RETRIES = 3
    ...
    def __init__(self):
        ...
        self.session.headers.update({
            ...
            'Origin': 'https://phedev.app',
            'Referer': 'https://phedev.app/',
    ...
    def _generate_key(self):
        ...
        return hashlib.pbkdf2_hmac('sha256', salt.encode(), b'phedev_salt', 100000, dklen=32)
    ...
    def _hwid(self):
        ...
        try:
            ...
            pass
        # Tạo HWID từ nhiều nguồn
        try:
            import winreg
            key = winreg.OpenKey(winreg.HKEY_LOCAL_MACHINE, r"SOFTWARE\Microsoft\Cryptography")
            machine_guid = winreg.QueryValueEx(key, "MachineGuid")[0]
            winreg.CloseKey(key)
            hw = hashlib.sha256(f"{machine_guid}{os.getenv('COMPUTERNAME')}".encode()).hexdigest()
        except:
            hw = hashlib.sha256(str(uuid.uuid4()).encode()).hexdigest()
        try:
            open(p, 'w').write(hw)
        except:
            pass
    ...
    def _post_json(self, url, data, encrypt=True):
        """Gửi request với mã hóa"""
        for attempt in range(1, self.MAX_RETRIES + 1):
            try:
                payload = self._encrypt_data(data) if encrypt else data
                resp = self.session.post(url, json={'data': payload} if encrypt else data, timeout=self.NET_TIMEOUT)
                if resp.status_code == 200:
                    try:
                        ...
                        return resp.json() if resp.text else {'status': 'error', 'message': 'Invalid response'}
    ...
    def check_key(self, key):
        ...
        result = self._post_json(self.BASE_URL + 'check', data)
        if result.get('status') == 'success':
            return {
                'valid': True,
                'expiry': result.get('expiry', 'forever'),
                'data': result,
                'user': result.get('username', ''),
                'device_limit': result.get('device_limit', 1)
            }
        return {'valid': False, 'message': result.get('message', 'Key không hợp lệ')}
    ...
    def get_key(self):
        ...
        result = self._post_json(self.BASE_URL + 'getkey', data)
        if result.get('status') == 'success':
            url = result.get('bypass_url') or result.get('shortenedUrl') or result.get('link')
            if url:
                return {'success': True, 'url': url, 'code': result.get('code', '')}
        return {'success': False, 'message': result.get('message', 'Không tạo được link')}
    ...
    def extend_key(self, key):
        ...
        result = self._post_json(self.BASE_URL + 'extend', data)
        if result.get('status') == 'success':
            return {'success': True, 'expiry': result.get('expiry', 'forever')}
        return {'success': False, 'message': result.get('message', 'Không thể gia hạn')}

    def save_license(self, key, data=None):
        """Lưu license"""
        try:
            save_data = {'key': key.strip()}
            if data:
                save_data['expiry'] = data.get('expiry', 'forever')
                save_data['user'] = data.get('user', '')
            json.dump(save_data, open(_path('license.json'), 'w'))
        except:
            pass
...
def check_debugger():
    try:
        ...
        # Check NtGlobalFlag
        peb = ctypes.c_void_p()
        ctypes.windll.ntdll.NtQueryInformationProcess(ctypes.windll.kernel32.GetCurrentProcess(), 0, ctypes.byref(peb), 8, None)
        if peb:
            nt_global = ctypes.cast(peb.value + 0x68, ctypes.POINTER(ctypes.c_long))
            if nt_global.contents.value & 0x70:
                _crack('NtGlobalFlag')
    except:
        pass
...
def check_monitor():
    bad = ['wireshark', 'fiddler', 'proxifier', 'charles', 'burp', 'httpanalyzer', 'netmon', 'packetmon', 'tcpview', 'processmonitor']
    for p in psutil.process_iter(['name']):
        try:
            name = p.info['name'] or ''
            if any(b in name.lower() for b in bad):
...
def check_api_hook():
    try:
        import win32api
        bad_hooks = ['api_monitor.dll', 'detours.dll', 'hook.dll', 'inject.dll']
        for hook in bad_hooks:
            try:
                if win32api.GetModuleHandle(hook):
                    _crack(f'API Hook: {hook}')
            except:
                pass
    except:
        pass

def check_anti_analysis():
    """Kiểm tra phân tích ngược"""
    try:
        # Check CPU count
        if psutil.cpu_count() < 2:
            _crack('Low CPU count')
        # Check RAM
        if psutil.virtual_memory().total < 2_000_000_000:  # 2GB
            _crack('Low RAM')
        # Check disk
        if psutil.disk_usage('/').total < 10_000_000_000:  # 10GB
            _crack('Low disk')
    except:
        pass
...
# ===== GIỮ NGUYÊN LOGIC CORE =====
# [Tất cả các hàm core từ file gốc: key_to_str, mouse_to_str, 
# save_keys, load_keys_file, EMULATOR_PATTERNS, find_emulator,
# FILTER_*, toggle_tele, toggle_freeze, toggle_ghost, toggle_aimlag,
# divert, send_packets, send_burst_packets, stop_all, quit_app]
    ...
    def _update(self):
        ...
        for ss in self.shooting_stars:
            if ss['active']:
                ss['x'] += ss['dx']
                ss['y'] += ss['dy']
                if ss['x'] > 400 or ss['x'] < 0 or ss['y'] > 320 or ss['y'] < 0:
                    ss['active'] = False
                ...
                if ss['timer'] <= 0:
                    ss['active'] = True
                    ss['x'] = random.randint(0, 200)
                    ss['y'] = random.randint(0, 100)
...
class NeoButton(QPushButton):
    def __init__(self, text, parent=None, glow_color='#cc0000'):
        ...
        self.setFont(F('Segoe UI', 11, True))
...
class NeoInput(QLineEdit):
    def __init__(self, placeholder='', password=False):
        ...
        self._glow = QGraphicsDropShadowEffect(self)
        self._glow.setBlurRadius(0)
        self._glow.setColor(QColor('#cc0000'))
        self._glow.setOffset(0, 0)
        self.setGraphicsEffect(self._glow)
...
class SplashScreen(CardWin):
    ...
    def __init__(self):
        super().__init__(420, 340, 18, False)
        body = self._body((0, 0, 0, 0))
        body.setContentsMargins(0, 0, 0, 0)
        body.setSpacing(0)
        
        self.starfield = StarField()
        self.starfield.setFixedSize(420, 340)
        ...
        logo = QLabel('PHEDEV')
        logo.setFont(F('Segoe UI', 30, True))
        logo.setStyleSheet('color:#ffffff;background:transparent;letter-spacing:8px;')
        logo.setAlignment(Qt.AlignmentFlag.AlignCenter)
        overlay_layout.addWidget(logo)
        
        sub = QLabel('DARK INFINITY • v4.0')
    ...
    def _tick(self):
        ...
        self._fill.setFixedWidth(int(280 * self._prog / 100))
...
# ===== LOGIN WINDOW =====
class LoginWindow(CardWin):
    login_success = pyqtSignal()
    _sig_ok = pyqtSignal()
    _sig_err = pyqtSignal(str)

    def __init__(self):
        super().__init__(400, 340, 18, True)
        ...
        self.starfield = StarField()
        self.starfield.setFixedSize(400, 340)
        body.addWidget(self.starfield)
        
        overlay = QWidget()
        overlay.setStyleSheet('background:rgba(0,0,0,130);border-radius:18px;')
        overlay_layout = QVBoxLayout(overlay)
        overlay_layout.setContentsMargins(28, 25, 28, 25)
        overlay_layout.setSpacing(8)
        
        title = QLabel('⚡ PHEDEV')
        title.setFont(F('Segoe UI', 22, True))
        title.setStyleSheet('color:#ffffff;background:transparent;letter-spacing:6px;')
        title.setAlignment(Qt.AlignmentFlag.AlignCenter)
        overlay_layout.addWidget(title)
        
        sub = QLabel('LICENSE ACTIVATION')
        sub.setFont(F('Segoe UI', 9, True))
        sub.setStyleSheet('color:#666666;background:transparent;letter-spacing:3px;')
        sub.setAlignment(Qt.AlignmentFlag.AlignCenter)
        overlay_layout.addWidget(sub)
        
        overlay_layout.addSpacing(10)
        
        self._kinp = NeoInput('Enter license key...', password=True)
        self._kinp.returnPressed.connect(self._go)
        overlay_layout.addWidget(self._kinp)
        
        btn_row = QHBoxLayout()
        btn_row.setSpacing(8)
        
        self._btn = NeoButton('ACTIVATE')
        self._btn.clicked.connect(self._go)
        btn_row.addWidget(self._btn)
        
        self._getkey_btn = NeoButton('GET KEY', glow_color='#666666')
        self._getkey_btn.clicked.connect(self._get_key)
        btn_row.addWidget(self._getkey_btn)
        
        overlay_layout.addLayout(btn_row)
        
        self._msg = QLabel('')
        self._msg.setFont(F('Segoe UI', 9, True))
        self._msg.setFixedHeight(20)
        self._msg.setAlignment(Qt.AlignmentFlag.AlignCenter)
        self._msg.setStyleSheet('background:transparent;')
        overlay_layout.addWidget(self._msg)
    ...
    def _load_saved(self):
        saved = keyapi.load_license()
        if saved:
            self._kinp.setText(saved)
            self._msg.setText('▶ Press ACTIVATE to continue')
            self._msg.setStyleSheet('color:#888888;background:transparent;')

    def _on_ok(self):
        self._msg.setText('✓ VALID')
        self._msg.setStyleSheet('color:#00ff66;background:transparent;')
        self._btn.setText('OK')
        self._btn.setEnabled(False)
        QTimer.singleShot(500, self.login_success.emit)

    def _on_err(self, msg):
        self._msg.setText(msg[:45])
        self._msg.setStyleSheet('color:#ff3333;background:transparent;')
        self._btn.setEnabled(True)
        self._btn.setText('ACTIVATE')
        self._getkey_btn.setEnabled(True)

    def _go(self):
        key = self._kinp.text().strip()
        if not key:
            self._msg.setText('⚠ Enter license key!')
            self._msg.setStyleSheet('color:#ff3333;background:transparent;')
            return
        self._msg.setText('⏳ Checking...')
        self._msg.setStyleSheet('color:#ffffff;background:transparent;')
        self._btn.setEnabled(False)
        self._btn.setText('CHECKING...')
        self._getkey_btn.setEnabled(False)
        
        def _run():
            try:
                result = keyapi.check_key(key)
                if result.get('valid'):
                    keyapi.save_license(key, result.get('data', {}))
                    self._sig_ok.emit()
                else:
                    self._sig_err.emit(result.get('message', 'Invalid key'))
            except Exception as ex:
                self._sig_err.emit(str(ex)[:40])
        threading.Thread(target=_run, daemon=True).start()

    def _get_key(self):
        self._getkey_btn.setEnabled(False)
        self._getkey_btn.setText('...')
        self._msg.setText('⏳ Generating link...')
        self._msg.setStyleSheet('color:#ffffff;background:transparent;')
        
        def _run():
            try:
                result = keyapi.get_key()
                if result.get('success'):
                    url = result.get('url')
                    threading.Thread(target=lambda: os.startfile(url), daemon=True).start()
                    self._msg.setText('🔗 Open browser, copy key!')
                    self._msg.setStyleSheet('color:#33d69f;background:transparent;')
                    QTimer.singleShot(3000, lambda: self._msg.setText('Paste key from browser'))
                else:
                    self._msg.setText(result.get('message', 'Failed'))
                    self._msg.setStyleSheet('color:#ff3333;background:transparent;')
            except Exception as ex:
                self._msg.setText(str(ex)[:40])
                self._msg.setStyleSheet('color:#ff3333;background:transparent;')
            finally:
                self._getkey_btn.setEnabled(True)
                self._getkey_btn.setText('GET KEY')
        threading.Thread(target=_run, daemon=True).start()
...
class PhedevESP(QMainWindow):
    def __init__(self):
        ...
        central = QWidget()
        central.setAttribute(Qt.WidgetAttribute.WA_TranslucentBackground)
        central.setStyleSheet('background:rgba(10,10,10,230);border-radius:10px;border:1px solid #1a1a1a;')
        self.setCentralWidget(central)
        ...
        # Mode buttons
        self.modes = ['Freeze', 'Ghost', 'Telekill', 'AimLag']
        self.mode_buttons = {}
        ...
        for mode in self.modes:
            ...
            def make_click(m, r, l, s):
                def handler(event):
                    self._click(m, r, l, s)
                return handler
            row.mousePressEvent = make_click(mode, row, lbl, status)
    ...
    def _click(self, mode, row, lbl, status):
        try:
...
def run():
    try:
        ...
        def show_login():
            lw[0] = LoginWindow()
            lw[0].login_success.connect(on_login)
            lw[0].show()

        def on_login():
            if lw[0]:
                lw[0].hide()
                QTimer.singleShot(50, lw[0].close)
            start_main()
        ...
        def start_main():
            ...
            def init_modules(found_hwnd, ww):
                ww.close()
                global waiting_window, taiwan_overlay, esp_window, mini_window, alive_timer
                waiting_window = None
                ...
                def on_ev(ev, down):
                    ...
                    if down:
                        ...
                        if binding_key != 'NONE' and kn == binding_key:
                            if hk_ref[0]:
                                QTimer.singleShot(0, hk_ref[0].toggle_vis)
                            return None
                        if kn == 'F10':
                            quit_app()
                            return None
